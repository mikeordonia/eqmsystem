...
router.get("/", (req,res)=>{
    res.render("appointment_cashier",{ans: "", number:"y", fullname:"", appstatus:"", sdate:"",
...
router.post('/', (req,res)=>{
    ...
    var sql = 'insert into appointment_cashier set ?'
    var query = conn.query(sql, data, (err, rset)=>{
        if(err){
            console.log(err)
            res.render('appointment_cashier')
        } else {
            ...
            var sql = "select bookid, name, status, schedDate, schedTime, purpose, bookDate, bookTime from appointment_cashier where name = ? and schedDate = ? and bookDate = ?"
            conn.query(sql,[name,schedDate,bookDate],(err,rset)=>{
                if (err)
                    console.log(err)
                else
                    console.log(rset)
                    res.render("appointment_cashier",{ans: "success", number:rset[0].bookid, 
