...
router.post("/", (req,res)=>{
    ...
    var sql = 'insert into queue_cashier set ?'
    console.log(data)
    var query = conn.query(sql,data, (err,rset)=>{
        if(err){
            ...
            var sql = "select queueNumber from queue_cashier where name = ? and qtime = ?"
            conn.query(sql,[name,qtime],(err,rset)=>{
                if (err)
                    console.log(err)
                else
                    console.log(rset)
                    res.render("home",{ans: "sQueueCashier", number:rset[0].queueNumber, fullname: name, qstatus: status, getqdate: qdate, getqtime: qtime})
            })
        }
    })
})
